  ...
  return (
    <article className="space-y-6">
      ...
      <header className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm sm:p-8">
        ...
        <h1 className="mt-4 mb-0 max-w-4xl text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl">
          {TITLE}
        </h1>
        <p className="mt-4 mb-0 max-w-3xl text-base leading-7 text-gray-700">
        ...
        <dl className="mt-5 grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-4">
          <div><dt className="font-semibold text-gray-500">Last updated</dt><dd className="mt-1 text-gray-900"><time dateTime={LAST_UPDATED}>July 17, 2026</time></dd></div>
          <div><dt className="font-semibold text-gray-500">Country</dt><dd className="mt-1 text-gray-900">China</dd></div>
          <div><dt className="font-semibold text-gray-500">Degree levels</dt><dd className="mt-1 text-gray-900">Bachelor&apos;s, Master&apos;s, PhD</dd></div>
          <div><dt className="font-semibold text-gray-500">Reading time</dt><dd className="mt-1 text-gray-900">10 minutes</dd></div>
        </dl>
        <div className="mt-6 flex flex-wrap gap-3">
          <a href="https://studyinchina.csc.edu.cn/" target="_blank" rel="noopener noreferrer" className="inline-flex items-center rounded-lg bg-gray-900 px-4 py-2 text-sm font-semibold text-white hover:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-gray-900 focus:ring-offset-2">
            Open CSC application portal <span aria-hidden="true">↗</span>
          </a>
          <Link href="/scholarship-results-2026" className="inline-flex items-center rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-semibold text-gray-900 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:ring-offset-2">
            View results hub
          </Link>
        </div>
      </header>
      ...
      <section className="rounded-2xl border border-blue-200 bg-blue-50 p-5 sm:p-6" aria-labelledby="quick-summary">
        <h2 id="quick-summary" className="mt-0 text-xl font-semibold text-gray-900">Quick summary</h2>
        <dl className="mt-4 grid gap-x-6 gap-y-4 text-sm sm:grid-cols-2">
          {[
            ...
            <div key={term}><dt className="font-semibold text-blue-900">{term}</dt><dd className="mt-1 text-gray-800">{detail}</dd></div>
          ))}
        </dl>
      </section>

      <section className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm sm:p-8">
        <h2 className="mt-0 text-2xl font-semibold text-gray-900">Are CSC Scholarship 2027 results out?</h2>
        <p className="mt-4 mb-0 leading-7 text-gray-700">
      ...
      <section className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm sm:p-8">
        <h2 className="mt-0 text-2xl font-semibold text-gray-900">CSC Scholarship 2027 result timeline</h2>
        <ol className="mt-5 grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-4">
          {["Application submitted", "University or dispatching-authority review", "CSC review and approval process", "Result notification", "Admission package", "Visa preparation", "Travel and registration"].map((step, index) => (
            <li key={step} className="rounded-xl border border-gray-200 bg-gray-50 p-4"><span className="block text-xs font-bold text-blue-700">STEP {index + 1}</span><span className="mt-1 block font-semibold text-gray-900">{step}</span></li>
          ))}
        </ol>
      </section>

      <section className="grid gap-4 lg:grid-cols-3">
        <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm"><h2 className="mt-0 text-xl font-semibold text-gray-900">Type A: embassy or dispatching-authority track</h2><p className="mt-3 mb-0 text-sm leading-6 text-gray-700">The authority that accepted or nominated the application normally controls communication. Monitor its website, email, and applicant instructions. Local review calendars and document delivery can make Type A dates vary by country.</p></div>
        <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm"><h2 className="mt-0 text-xl font-semibold text-gray-900">Type B: university track</h2><p className="mt-3 mb-0 text-sm leading-6 text-gray-700">A university may issue pre-admission, nomination, or internal selection information before final scholarship confirmation. Treat a university recommendation as provisional until the responsible authority provides the final result and admission documents.</p></div>
        <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm"><h2 className="mt-0 text-xl font-semibold text-gray-900">Type C: other designated routes</h2><p className="mt-3 mb-0 text-sm leading-6 text-gray-700">Type C can refer to other application channels identified in a specific programme notice. Follow it only when the official call explicitly gives a Type C agency number and notification process.</p></div>
      </section>

      <section className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm sm:p-8">
        <h2 className="mt-0 text-2xl font-semibold text-gray-900">How to check your CSC Scholarship result</h2>
        <div className="mt-5 grid gap-4 sm:grid-cols-2">
          {[
            ...
            <div key={heading} className="rounded-xl border border-gray-200 bg-gray-50 p-4"><h3 className="m-0 text-base font-semibold text-gray-900">{heading}</h3><p className="mt-2 mb-0 text-sm leading-6 text-gray-700">{text}</p></div>
          ))}
        </div>
      </section>

      <section className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
        <div className="p-6 pb-3 sm:px-8"><h2 className="mt-0 text-2xl font-semibold text-gray-900">CSC result status meanings</h2><p className="mt-2 mb-0 text-sm text-gray-600">These are practical interpretations, not a guarantee that every portal or authority uses identical labels.</p></div>
        <div className="overflow-x-auto"><table className="w-full min-w-[640px] text-left text-sm"><thead className="bg-gray-50 text-gray-700"><tr><th className="px-6 py-3 font-semibold sm:px-8">Status</th><th className="px-6 py-3 font-semibold">What it generally means</th></tr></thead><tbody className="divide-y divide-gray-200">{statusRows.map(([status, meaning]) => <tr key={status}><th scope="row" className="px-6 py-3 font-semibold text-gray-900 sm:px-8">{status}</th><td className="px-6 py-3 text-gray-700">{meaning}</td></tr>)}</tbody></table></div>
      </section>

      <section className="grid gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm"><h2 className="mt-0 text-2xl font-semibold text-gray-900">What happens after selection?</h2><ol className="mt-4 mb-0 list-decimal space-y-2 pl-5 text-sm leading-6 text-gray-700"><li>Confirm that the notice is final and check the scholarship coverage.</li><li>Receive or download the admission notice and route-specific documents.</li><li>Prepare the visa application form, including JW201 or JW202 when issued and applicable.</li><li>Complete the required physical examination and visa process.</li><li>Follow the university&apos;s registration, accommodation, and arrival instructions.</li><li>Travel to China only after the visa and admission arrangements are confirmed.</li></ol></div>
        <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm"><h2 className="mt-0 text-2xl font-semibold text-gray-900">Documents commonly needed after selection</h2><ul className="mt-4 mb-0 list-disc space-y-2 pl-5 text-sm leading-6 text-gray-700"><li>Valid passport</li><li>Official admission or scholarship notice</li><li>Visa application documents supplied for the case</li><li>Foreigner physical examination documentation when required</li><li>Original or authenticated degree certificates and transcripts</li><li>Police clearance if requested by the university or visa authority</li><li>Passport photographs meeting the stated specifications</li></ul><p className="mt-4 mb-0 text-sm font-medium text-gray-800">Use the final university and visa instructions as the controlling checklist.</p></div>
      </section>

      <section className="rounded-2xl border border-amber-200 bg-amber-50 p-6 sm:p-8">
        <h2 className="mt-0 text-2xl font-semibold text-gray-900">If you have not received your result yet</h2>
        <p className="mt-3 mb-0 leading-7 text-gray-700">Do not assume silence means rejection. Universities and dispatching authorities release decisions at different times, and administrative processing can continue after another institution has announced its list. Check official channels regularly, confirm that your contact details are correct, and avoid sending repeated daily emails.</p>
        <h3 className="mt-5 text-lg font-semibold text-gray-900">Common reasons for a delay</h3>
        <ul className="mt-3 mb-0 grid gap-2 text-sm text-gray-700 sm:grid-cols-2"><li>Additional document or identity verification</li><li>CSC approval still pending</li><li>University nomination processing</li><li>Embassy or dispatching-authority scheduling</li><li>Public holidays and administrative closures</li><li>Missing or inconsistent documents</li></ul>
      </section>

      <section className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm sm:p-8">
        <h2 className="mt-0 text-2xl font-semibold text-gray-900">Frequently asked questions</h2>
        <div className="mt-5 space-y-5">{faqs.map((faq) => <div key={faq.question}><h3 className="m-0 text-base font-semibold text-gray-900">{faq.question}</h3><p className="mt-2 mb-0 text-sm leading-6 text-gray-700">{faq.answer}</p></div>)}</div>
      </section>

      <section className="grid gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm"><h2 className="mt-0 text-2xl font-semibold text-gray-900">Official resources</h2><ul className="mt-4 mb-0 space-y-3 text-sm"><li><a href="https://www.campuschina.org/" target="_blank" rel="noopener noreferrer" className="font-semibold text-blue-700 hover:underline">Campus China / China Scholarship Council ↗</a></li><li><a href="https://studyinchina.csc.edu.cn/" target="_blank" rel="noopener noreferrer" className="font-semibold text-blue-700 hover:underline">CSC application system ↗</a></li><li><a href="https://www.studyinchina.edu.cn/" target="_blank" rel="noopener noreferrer" className="font-semibold text-blue-700 hover:underline">Study in China portal ↗</a></li><li><span className="font-semibold text-gray-900">Your university:</span> use its official international admissions website.</li><li><span className="font-semibold text-gray-900">Your embassy or dispatching authority:</span> use the exact office named in the Type A call.</li></ul></div>
        <div className="rounded-2xl border border-blue-200 bg-blue-50 p-6"><h2 className="mt-0 text-2xl font-semibold text-gray-900">Key takeaways</h2><ul className="mt-4 mb-0 list-disc space-y-2 pl-5 text-sm leading-6 text-gray-700"><li>CSC Scholarship 2027 results are not currently released.</li><li>July–August 2027 is an expected monitoring period, not an official universal promise.</li><li>Type A and Type B use different notification paths.</li><li>Pre-admission or recommendation does not necessarily equal final CSC approval.</li><li>Rely on CSC, your university, embassy, or dispatching authority—not social-media rumors.</li></ul></div>
      </section>

      <section className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm sm:p-8">
        <h2 className="mt-0 text-2xl font-semibold text-gray-900">Related resources</h2>
        <div className="mt-4 grid gap-3 sm:grid-cols-2">{relatedLinks.map((item) => <Link key={item.href} href={item.href} className="rounded-xl border border-gray-200 bg-gray-50 p-4 text-sm font-semibold text-gray-900 hover:bg-white focus:outline-none focus:ring-2 focus:ring-blue-600 focus:ring-offset-2">{item.label} <span aria-hidden="true">→</span></Link>)}</div>
      </section>

      <footer className="rounded-2xl border border-gray-200 bg-gray-50 p-5 text-sm text-gray-600">
        <p className="m-0"><strong className="text-gray-900">Editorial note:</strong> Published <time dateTime={PUBLISHED_DATE}>July 17, 2026</time>; last reviewed <time dateTime={LAST_UPDATED}>July 17, 2026</time>. Sources reviewed: Campus China, the CSC application system, and Study in China. This page separates confirmed information from editorial timing estimates and should be updated as official 2027 announcements appear.</p>
      </footer>
    </article>
  );
}
